...
class BoardController: NSObject,
  ...
  private func applyNumGuessesSettings(with settings: [String: Any]) {
      if let numGuesses = settings[kNumGuessesKey] as? Int {
              // Set the 'numRows' property to the retrieved value
              numRows = numGuesses
          } else {
              // Handle the case where 'kNumGuessesKey' is not found or its value is not an integer
              print("Invalid or missing value for \(kNumGuessesKey)")
          }
      }
  }
  ...
  private func applyThemeSettings(with settings: [String: Any]) {
      if let themeString = settings[kWordThemeKey] as? String {
              // Convert the themeString to a WordTheme using the WordTheme(rawValue:) initializer
          if let theme = WordTheme(rawValue: themeString) {
              // Generate a new goal word based on the theme and assign it to the 'goalWord' property
              let goalWord = WordGenerator.generateGoalWord(with:theme)
              
          } else {
                  // Handle the case where the themeString is not a valid WordTheme
                  print("Invalid theme: \(themeString)")
              }
          } else {
              // Handle the case where 'kWordThemeKey' is not found or its value is not a String
              print("Invalid or missing value for \(kWordThemeKey)")
          }
  }
...
private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // Check if the 'kIsAlienWordleKey' exists in the settings dictionary
    if let isAlienWordleSetting = settings[kIsAlienWordleKey] as? Bool {
        // Set the 'isAlienWordle' property to the retrieved value
        let isAlienWordle = isAlienWordleSetting
        
    } else {
        // Handle the case where 'kIsAlienWordleKey' is not found or its value is not a Bool
        print("Invalid or missing value for \(kIsAlienWordleKey)")
    }
}
